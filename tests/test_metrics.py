...
# Hand-computed toy case, worked out on paper (see build notes):
...
def test_recall_with_partial_gold_coverage():
    retrieved = ["A", "X", "Y"]
    gold = {"A", "B"}  # only A is retrieved, B is missed entirely
    assert recall_at_k(retrieved, gold) == 0.5
    assert hit_rate(retrieved, gold) == 1.0


def test_em_and_f1_basic():
    assert exact_match("The answer is 400 days.", "400 days") == 0.0  # extra words -> not exact
    assert exact_match("400 days", "400 Days") == 1.0  # case-insensitive
    assert exact_match("a 12 hours", "12 hours") == 1.0  # article stripped
    ...
    assert 0.0 < f1 < 1.0  # partial overlap
